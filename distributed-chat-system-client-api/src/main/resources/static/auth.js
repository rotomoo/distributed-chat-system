...
document.getElementById('signup-form').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch('/v1/public/auth/signup', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            messageElement.textContent = `에러: ${error.message || '회원가입에 실패했습니다.'}`;
        }
    } catch (error) {
...
document.getElementById('login-form').addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            messageElement.style.color = 'green';
            messageElement.textContent = `로그인 성공! 토큰: ${data.data.token}`;
        } else {
            const error = await response.json();
            messageElement.style.color = 'red';
            messageElement.textContent = `에러: ${error.message || '로그인에 실패했습니다.'}`;
        }
    } catch (error) {
        console.error('Error:', error);
        messageElement.style.color = 'red';
